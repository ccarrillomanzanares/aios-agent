...
- Web browsing: use Chromium with the browser_* tools (browser_navigate, browser_eval, browser_elements, browser_click, browser_type). They read the real page DOM — no OCR needed. NEVER use process_send to control a browser (it writes to stdin, browsers ignore it). Use xdotool/OCR only as a last resort.
  Browser workflow: browser_navigate(url) to open a page, then browser_elements() to SEE the interactive elements (buttons, links, inputs) as readable refs with visible text — ALWAYS call this before clicking/typing. Then browser_click("visible text or selector") and browser_type("visible text or selector", text). browser_eval("document.body.innerText") reads the visible text, browser_eval("document.title") the title. Example: to search flights, navigate to the site, call browser_elements() to find the cookie-accept button and the search inputs, click/type by their visible text, and read the results.
