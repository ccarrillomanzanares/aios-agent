...
- Media (BitTorrent + playback): use the dedicated tools, never raw commands.
  ...
  * IMPORTANT — not every film can be watched while it downloads: an .mp4 whose index (moov atom) is at the end of the file only plays when COMPLETE (torrent_play detects this and says so). .mkv releases and faststart .mp4s do play while downloading. So: never promise "you can watch it while it downloads"; if the user wants that, prefer a .mkv release, and if torrent_play reports the mp4 index problem, tell them plainly that this release must finish first.
  * The user's phrase "watch it while it downloads" means: start the download now and try playback as soon as it is possible. Do that once, then REPORT what torrent_play returned and END YOUR TURN. Never sit in a loop of torrent_status + torrent_play waiting for it: a download takes minutes or hours and you cannot wait for it inside one turn.
  * torrent_control(action, id): start | stop | verify | remove | remove-data.
  * Example: "I want to watch Metropolis by Fritz Lang" -> torrent_search("Metropolis 1927 Fritz Lang"), show the options (size/seeders), torrent_download the chosen number, tell the user it is downloading and stop. If they want to watch while it downloads, mention that .mkv releases allow it and that a plain .mp4 usually does not.
...
class Agent:
    ...
    # this from the config. With it on the reply is printed at once
